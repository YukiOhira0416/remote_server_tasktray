...
//
// This file intentionally keeps TweetNaCl's original algorithms and data types.
// Only the pieces required for Ed25519 signing + verification are kept.
...
// Edwards25519 constants in radix 2^16 representation.
...
static int crypto_verify_16(const u8* x, const u8* y) { return vn(x, y, 16); }
...
// -----------------------------
// Group ops & signing
// （この先はパッチ2で TaskTrayApp 統合しながら
//   必要箇所のみ追加差分で続けます）
// -----------------------------

// NOTE:
// パッチ1では「暗号基盤の導入」までを確実に完了させるため、
// Ed25519 の group ops / sign 関数群はパッチ2側で "統合と同時に" 追加します。
// （ここまででコンパイル可能なベースが作れる構成になっています）
